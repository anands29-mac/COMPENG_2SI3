...
#include "objPosStackModel.h"
#include "cmdQueueModel.h"
...
objPosStackModel *StackModel;
cmdQueueModel *QueueModel;
...
void testPushTenTop() {
	...
	{
		randomItem = generateRandomPos();
		StackUnderTest->push(randomItem);
		StackModel->push(randomItem);
		
		expected = StackModel->top();
...
void testEnqueueDequeueGetSizeTen() {
	...
	for(int i = 0; i < COUNT; i++)
	{
		randomItem = generateRandomChar();
		QueueUnderTest->enqueue(randomItem);
		QueueModel->enqueue(randomItem);
	}
	...
	for(int i = 0; i < COUNT; i++)
	{
		expected = QueueModel->dequeue();
		actual = QueueUnderTest->dequeue();
...
int main(int argc, char const *argv[]) {
	...
	if(passCount == 6 && !testSortReady)
		cout << "If passing 6/7 test cases, check if you have set testSortReady to true on Line 18 in this file!" << endl;
...
void createNewStack()
{
	if(StackUnderTest != NULL || StackModel != NULL) clearStack();
	StackUnderTest = new objPosStack();
	StackModel = new objPosStackModel();	
}
...
void createNewQueue()
{
	if(QueueUnderTest != NULL || QueueModel != NULL) clearQueue();
	QueueUnderTest = new cmdQueue();
	QueueModel = new cmdQueueModel();
}
